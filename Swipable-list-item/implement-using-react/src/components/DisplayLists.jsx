...
function DisplayLists() {
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100vh' }}>
      ...
      {toast.show && (
        <div className={`toast ${toast.type} ${toastHiding ? 'hide' : ''}`} role="status" aria-live="polite">
          <span>{toast.message}</span>
          <button className="close-btn" onClick={() => hideToast()} aria-label="Close toast">✕</button>
        </div>
      )}
    </div>
  );
